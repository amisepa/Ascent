...
load(fullfile(data_path, sprintf('ascent_outputs_EC_%s.mat', coarsing)), ...
     'SampEn','FuzzEn','ExSEnt1','ExSEnt2','ExSEnt3','FracDim','MSE','MFE','mMSE','RCMFE','scales', 'scales_bounds')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PARAMETERS %%%%%%%%%%%%%%%%%%%%%%%%%%%%
nPerm = 10000;          % number of permutations for H0
...
set(findall(gcf, 'type', 'axes'), 'FontSize', 15, 'FontWeight', 'bold');
...
if ~isempty(mask_clusters)
    ...
    hs = plot_clusters(summary_tbl, mask_clusters, tvals, scales, chanlocs, 'MSE', 'DataType', 'scalp', 'LineNoiseHz', []);
...
mask = compute_mcc(tvals, pvals, tvals_H0, pvals_H0, mcc_type, alpha, chanlocs);
[mask_clusters, summary_tbl] = pull_clusters(mask, tvals, scales, chanlocs, ...
    'nonlinear', grp_type, {size(mMSE1,3) size(mMSE2,3)}, [], [], [], 'cohen-d');
plot_results('nonlinear', 'scalp', scales, tvals, mask_clusters, chanlocs, 'main', summary_tbl);
title("mMSE")
set(findall(gcf, 'type', 'axes'), 'FontSize', 16, 'FontWeight', 'bold');
...
if ~isempty(mask_clusters)
    ...
    hs = plot_clusters(summary_tbl, mask_clusters, tvals, scales, chanlocs, 'mMSE', 'DataType', 'scalp', 'LineNoiseHz', []);
...
if ~isempty(mask_clusters)
    ...
    hs = plot_clusters(summary_tbl, mask_clusters, tvals, scales, chanlocs, 'MFE', 'DataType', 'scalp', 'LineNoiseHz', []);
    % xticklabels(scales_bounds);
    for i = 1:numel(hs.curve)
...
if ~isempty(mask_clusters)
    ...
    hs = plot_clusters(summary_tbl, mask_clusters, tvals, scales, chanlocs, 'RCMFE', 'DataType', 'scalp', 'LineNoiseHz', []);
    xticklabels(scales_bounds);
    for i = 1:numel(hs.curve)
