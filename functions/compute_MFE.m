...
if parallelMode && ~isempty(ver('parallel'))
    parfor ch = 1:nch
        MFE(ch,:) = mfe_one_channel(Xz(ch,:), m, r, n_exp, tau, coarseType, ...
            S, minBinsAll, minBinsStable, blockSize, pdistMaxGB, false, ch, nch);
        if useDQ
            send(dq, 1);
        end
    end
    ...
    for ch = 1:nch
        MFE(ch,:) = mfe_one_channel(Xz(ch,:), m, r, n_exp, tau, coarseType, ...
            S, minBinsAll, minBinsStable, blockSize, pdistMaxGB, showProgress, ch, nch);
